 ...
 * 
 * @struts.action path="/ProcessCreateQuestion" name="CreateQuestionForm"
 *                input="/quiz/Question.jsp" scope="session" validate="true"
 * 
 * @struts.action-forward name="DisplayCreateQuestion"
 *                        path="/DisplayCreateQuestion.do" redirect="false"
 * 
 * @spring.bean name="/ProcessCreateQuestion"
 * 
...
public class ProcessCreateQuestion extends AhpAbstractProcessAction {
	...
	public ActionForward process(ActionMapping pActionMapping,
		...
		Quiz lQuiz = (Quiz) pHttpServletRequest
		...
		if (!lCreateQuestionForm.isSubmitAction(SubmitActions.EMPTY)) {
			if (lQuiz != null) {
				...
				lCreateQuestionForm
						.setNextPage(NavigateActions.DisplayCreateQuestionStart
								.toString());
			}
			if (lCreateQuestionForm.isSubmitAction(SubmitActions.ADD_OPTION)
				...
				lCreateQuestionForm
						.setNextPage(NavigateActions.DisplayCreateQuestion
								.toString());
			}
			...
			if (lCreateQuestionForm.isSubmitAction(SubmitActions.CANCEL)) {
				lCreateQuestionForm
						.setNextPage(NavigateActions.DisplayCreateQuizCancelled
								.toString());
			}
		} else {
			lCreateQuestionForm
		...
		return pActionMapping.findForward(NavigateActions.DisplayCreateQuestion
				.toString());
	}
	 ...
	 * @param pLoggedInUser
	 */
	private void storeQuestion(CreateQuestionForm pCreateQuestionForm,
			User pLoggedInUser) {
		...
		lQuestion.setQuestionType(QuestionType.valueOf(pCreateQuestionForm
				.getQuestionType()));
		lQuestion.setQuestionDescription(pCreateQuestionForm
				.getQuestionDescription());
		lQuestion.setQuestionObjective(pCreateQuestionForm
				.getQuestionObjective());
		if (lQuizUnderCreation.getQuestions() != null) {
			lQuestion.setQuestionOrder(pCreateQuestionForm.getQuiz()
					.getQuestions().size() + 1);
		} else {
			lQuestion.setQuestionOrder(1);
		}
		Category lCategory = new Category();
		lCategory.setCategoryId(pCreateQuestionForm
				.getSelectedQuestionCategory());
		lQuestion.setCategory(lCategory);

		SkillLevel lSkillLevel = new SkillLevel();
		lSkillLevel.setSkillLevelId(pCreateQuestionForm
		...
		if (StringUtils.isNotBlank(pCreateQuestionForm
				.getResponseDurationInHours())) {
			lQuestionDuration += TimeUnit.MILLISECONDS.convert(
					Long.parseLong(pCreateQuestionForm
