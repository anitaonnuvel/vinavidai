...
--%>
                        <tr>
                            <td colspan="${form.paginationData.tableColSpan}" class="ahpPaginationTableWhite">
                                <span style="float:left">
	                                <label>Sort By</label>
	                                <html:select name="${form.formName}" 
	                                    property="paginationData.sortByParam" 
	                                    size="1">
	                                    <html:options name="${form.formName}" 
	                                        property="paginationData.sortByParamSet"/> 
	                                </html:select> 
	                                <label>Sort Order</label>
	                                <html:select name="${form.formName}" 
	                                    property="paginationData.sortByOrder" 
	                                    size="1">
	                                    <html:options name="${form.formName}" 
	                                        property="paginationData.sortByOrderSet"/> 
	                                </html:select> 
	                                <html:submit property="submitAction" value="Go"></html:submit>
                                </span>
                                <c:if test="${form.paginationData.totalRecords > form.paginationData.maxRecordsPerPage}">
	                                <span style="float:right">
		                                ...
		                                <html:select styleId="resultsRangeComboTop" name="${form.formName}" property="paginationData.selectedPage" onchange="javascript:submitPage(document.getElementById('resultsRangeComboTop').value)">>
		                                   <html:optionsCollection name="${form.formName}" property="paginationData.comboPageRange" label="pageRange" value="pageIndex" />
		                                </html:select> 
